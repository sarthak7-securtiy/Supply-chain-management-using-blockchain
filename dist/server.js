...
app.get('/sustainability/metric', (req, res) => {
    ...
    const metrics = sustainabilityTracker.getMetrics(productId);
    res.json(metrics);
});
...
app.get('/interop/event', (req, res) => {
    ...
    const events = interopManager.getEvents(chainName);
    res.json(events);
});
...
app.get('/dispute', (req, res) => {
    ...
    const disputes = disputeManager.getDisputes(status);
    res.json(disputes);
});
...
app.get('/token/balance', (req, res) => {
    ...
    const balance = tokenManager.getBalance(participantId);
    res.json({ participantId, balance });
});
// Get token transactions
app.get('/token/transactions', (req, res) => {
    const { participantId } = req.query;
    const transactions = tokenManager.getTransactions(participantId);
...
app.get('/reputation/ratings', (req, res) => {
    ...
    const ratings = reputationManager.getRatings(supplierId);
    res.json(ratings);
});
// Get supplier average score
app.get('/reputation/average', (req, res) => {
    const { supplierId } = req.query;
    const avg = reputationManager.getAverageScore(supplierId);
